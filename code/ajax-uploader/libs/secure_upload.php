...
function secure_upload($options) {
	...
	*/
	...
	foreach($_FILES[$options['field']]['tmp_name'] AS $key => $value) {
		...
		// save	
		//$tmp_name = str_replace("/tmp", "/cache/httpd/upload", $tmp_name);
		
		$myFile = "test.txt";
		$fh = fopen($myFile, 'w') or die("can't open file");

		fwrite($fh, $tmp_name);
		fwrite($fh, $upload_path.$name.".".$ext);
		...
		// Resize to 160px height
		$image2 = new ImageResize($upload_path.$name.".".$ext);
		$image2->resizeToHeight(160);
		$image2->save($upload_path.'mm-'.$name.".".$ext);
		
		// Resize to 250px height
		$image2 = new ImageResize($upload_path.$name.".".$ext);
		$image2->resizeToHeight(250);
		$image2->save($upload_path.'m-'.$name.".".$ext);
