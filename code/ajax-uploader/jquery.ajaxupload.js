...
(function ($) {
	...
	$.ajaxUploadPrompt = function( options ) {
		...
		var d = $('<input type="file" name="' + $.ajaxUploadSettings.name + '" />').appendTo(form);
		d.change(function() {			
	...
	$.fn.ajaxUploadPrompt = function( origSettings ) {
		this.each(function() {
			if (needOverlay) {
				...
				var d = $('<input type="file" name="' + $.ajaxUploadSettings.name + '" style="position: absolute; top: 0; right: 0; font-size: ' + $this.outerHeight() + 'px; cursor: pointer;" />').appendTo(p);
				d.css({
					opacity: 0
				});
				d.change(function() {
